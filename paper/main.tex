...
\input{paper_b_generated_metrics.tex}
...
\begin{equation}
T_{\text{FU}} = \frac{N n_f}{\text{FPS}}, \quad \Theta = \frac{\text{FPS}}{n_f} \times 3{,}600 \text{ units/hr}.
\end{equation}
For the baseline single-view configuration ($n_f = 1.0$) at $\text{FPS} = 30.0$, $T_{\text{FU}} = 33.33$\,s and $\Theta = 108{,}000$\,parts/hour.
...
\begin{enumerate}
    \item \textbf{Active Edge Compute Infrastructure:} Bounded in scope to the duty-cycle-averaged NVIDIA RTX 4050 Laptop GPU power measured via NVML at 100\,Hz host cadence across continuous inference, Cost-Calibrated Thresholding (CCT), and temporal cascade filtering (host platform CPU/RAM and camera power are excluded from direct GPU telemetry and captured in cell baseline overhead).
...
\begin{enumerate}
    ...
    \begin{equation}
    E_{\text{edge}} = \frac{N n_f e_{\text{frame}}}{1{,}000} \quad [\text{kWh / FU}].
    \end{equation}
    where $e_{\text{frame}}$ is the calibrated active edge energy per frame in Wh/frame (or $10^3$\,mWh/frame, derived from physical frame energy in Joules as $J / 3{,}600$). For our calibrated single-view workcell ($n_f = 1.0$) drawing active GPU power $P_{\text{active}} \approx 13.71$\,W at $30$\,FPS, yielding per-frame energy $e_{\text{frame}} = 13.71/30 = 0.457$\,J ($1.27\times 10^{-4}$\,Wh / $0.127$\,mWh), giving $E_{\text{edge}} = 0.127$\,kWh per functional unit.
    \item \textbf{Human Review Workstation Energy ($E_{\text{review}}$):} Let $\lambda_p$ be the hourly alert arrival rate emitted by policy $p$. The number of alerts generated during the inspection of $N = 1{,}000$ units is:
    \begin{equation}
    n_{\text{alerts}} = \lambda_p \times \frac{N}{\Theta} = \lambda_p \times \frac{N n_f}{\text{FPS} \times 3{,}600}.
    \end{equation}
...
The net greenhouse gas benefit relative to baseline decomposes into five additive operational waterfall drivers (visualized across scenarios in Fig.~\ref{fig:carbon_waterfall}):
\begin{equation}
...
\end{equation}
subject to $\sum w_i = 1.0$ and $w_i \ge 0$ (profile geometries are contrasted in Fig.~\ref{fig:sqi_comparison}). Across four stakeholder priority profiles defined in our configuration ledger:
\begin{itemize}
...
When throttled to the line capture rate of $30.0$\,FPS with duty-cycled SQLite WAL spooling, the integrated duty-cycle-averaged RTX 4050 GPU-device power (polled via NVML at 100\,Hz host cadence; host platform/camera/storage power excluded from direct GPU telemetry term) averaged $13.71$\,W, establishing an empirical continuous frame energy of $e_{\text{frame}} = \MeasuredFrameEnergyWh$ (\MeasuredFrameEnergyJoules), or $E_{\text{edge}} = \CalibratedEdgeEnergyKWh$ per $1{,}000$ inspected parts.

\input{tables/tab2_policy_hierarchy_sustainability.tex}
...
Table~\ref{tab:policy_sustainability} and Fig.~\ref{fig:raw_outcomes} detail the sustainability outcomes across the 5-tier policy hierarchy. Policies B0 through B4 are evaluated through a single unified simulation and accounting pipeline code path evaluating structured scenario policy mappings from raw anomaly scores through CCT calibration and temporal gating:
\begin{enumerate}
...
\input{tables/tab3_monte_carlo_confidence.tex}
...
\input{tables/tab4_break_even_frontiers.tex}
...
\begin{figure*}[tbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig7_waterfall_carbon_decomposition.pdf}
\caption{Net carbon waterfall decomposition ($\Delta C = \Delta C_{\text{mat}} + \Delta C_{\text{rw}} + \Delta C_{\text{rev}} - C_{\text{edge}} + \Delta C_{\text{esc}}$) across Scenarios A, B, and C under Policy $B4$. Defect escape avoidance ($\Delta C_{\text{esc}}$) constitutes the primary positive contributor (scaling proportionally with embodied escape penalties across Scenarios A, B, and C), while continuous edge compute electricity ($-C_{\text{edge}} = -0.071$\,kgCO$_2$e) forms a persistent negative offset.}
\label{fig:carbon_waterfall}
\end{figure*}

\begin{figure*}[tbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig8_adverse_sensitivity_sweeps.pdf}
\caption{Adverse operational sensitivity sweeps showing empirical break-even frontiers: (Left) Net carbon $\Delta C$ as defect prevalence $\pi \to 0$, identifying net-negative carbon crossover at $\pi^\star = 0.0053\%$ ($0.053$ defects/1k parts) for Scenario A and $\pi^\star = 0.00093\%$ for Scenario B; (Right) Sensitivity to multi-view camera loading $n_f \in [1, 10]$ frames/part, illustrating linear compute energy escalation to $1.71$\,kWh/1k parts.}
\label{fig:adverse_sensitivity}
\end{figure*}

\section{Discussion, Negative Regimes \& Threats to Validity}
\subsection{When Is Edge AI Not Green?}
Our findings directly challenge the universal assumption that edge AI inspection is always environmentally beneficial. As empirically demonstrated in Fig.~\ref{fig:adverse_sensitivity}, we identify three distinct operational regimes where edge AI is \textbf{net-negative}:
\begin{enumerate}
    \item \textbf{Ultraclean High-Volume Production ($\pi \to 0$):} In mature manufacturing lines operating under Six Sigma ($<3.4$\,defects per million), continuous AI vision systems consume continuous kilowatt-hours while intercepting virtually zero defects, producing a net carbon deficit ($\Delta C < 0$, e.g., $-0.071$\,kgCO$_2$e per $1{,}000$ units in Scenario~A, Fig.~\ref{fig:adverse_sensitivity} left).
    ...
    \item \textbf{Multi-View Inspection Overhead ($n_f \ge 10$):} If complex part geometries require multi-angle camera inspection ($n_f = 10$ frames/part), continuous edge compute energy scales tenfold ($E_{\text{edge}} = 1.71$\,kWh/1k parts), narrowing the carbon payback margin (Fig.~\ref{fig:adverse_sensitivity} right).
...
While this study establishes an operational framework for evaluating edge inspection trade-offs, several limitations must be noted. First, active compute power was profiled on an NVIDIA RTX 4050 mobile GPU; embedded NPUs or specialized edge ASICs may exhibit lower quiescent power draw. Second, policy operating parameters ($\lambda_p, r_p, d_p$) were treated as scenario-driven configurations rather than dynamic, data-driven closed-loop estimators. Finally, the analysis relies on deterministic review service times, whereas human ergonomic fatigue introduces stochastic variance in high-workload queues. Future work will integrate live industrial floor data streams and multi-platform hardware profiling.
