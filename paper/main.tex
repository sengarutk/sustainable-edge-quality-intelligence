...
\input{../results/paper_b_generated_metrics.tex}
...
While academic benchmarks emphasize ROC-AUC and PRO metrics, Flagship~1~\cite{sengar2026flagship1} demonstrated that standard thresholds produce intolerable false positive rates in operational regimes. Flagship~4 / Paper~A~\cite{sengar2026paperA} resolved this by introducing Cost-Calibrated Thresholding (CCT) combined with a 7-stage temporal cascade (Dual-EMA filtering, $k$-of-$N$ sliding window persistence, and state-gated cooldown), suppressing false alarms by up to 93.3\% while bounding median detection delay to 3.0 frames. However, Paper~A focused solely on operational reliability and alert fatigue, leaving the environmental and material consequences unmeasured.
...
Defect Prevalence ($\pi_A, \pi_B, \pi_C$) & $\pi$ & 0.04, 0.03, 0.015 & $\pm 25\%$ & fraction & Scenario assumption & ISO 2859-1~\cite{iso2859} & Lot defect fraction per functional unit \\
Embodied Carbon ($EF_{\text{mat}}$) & $EF$ & 3.5, 8.24, 22.5 & $\pm 15\%$ & kgCO$_2$e/kg & Literature-derived & ecoinvent v3.9~\cite{ashby2012materials} & Cradle-to-gate primary material carbon factor \\
...
C_{\text{escape}} &= N_{\text{escape}} \cdot C_{\text{escape}},
\end{align}
where $\gamma = 0.417$\,kgCO$_2$e/kWh is the official EU-27 average grid electricity carbon factor~\cite{eea2024greenhouse}, and $C_{\text{escape}}$ is the lifecycle carbon penalty of an escaped defective unit (field returns, warranty logistics, pack disassembly).

The net greenhouse gas benefit relative to the uncalibrated baseline is:
\begin{equation}
\Delta C = C_{\text{baseline}}^{\text{total}} - C_p^{\text{total}} \quad [\text{kgCO}_2\text{e}].
\end{equation}
A positive $\Delta C$ indicates an absolute carbon reduction.

\subsection{Operator Workload \& Queue Utilization}
To model human sustainability and cognitive fatigue, the manual review station is treated as a queueing system with service capacity $\mu = 60.0$\,reviews/hour. The queue traffic intensity is:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Ultraclean High-Volume Production ($\pi \to 0$):} In mature manufacturing lines operating under Six Sigma ($<3.4$\,defects per million), continuous AI vision systems consume continuous kilowatt-hours while intercepting virtually zero defects, increasing net factory emissions.
    \item \textbf{Uncalibrated Cloud-Streaming Workcells ($e_{\text{edge}} > e^\star$):} Offloading raw video frames to remote cloud GPUs over cellular/Wi-Fi interfaces expands data transmission energy, rapidly exceeding the allowable budget ($127.05$\,kWh/1k).
