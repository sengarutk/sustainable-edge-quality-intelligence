...
\begin{enumerate}
    ...
    \end{equation}
    where $e_{\text{frame}}$ is the calibrated active edge energy per frame in Wh/frame (or $10^3$\,mWh/frame, derived from physical frame energy in Joules as $J / 3{,}600$). For our calibrated single-view workcell ($n_f = 1.0$) drawing duty-cycle-averaged GPU active power $P_{\text{active}} \approx 18.5$\,W over $33.33$\,s, $E_{\text{edge}} = \CalibratedEdgeEnergyKWh$ ($e_{\text{frame}} = \MeasuredFrameEnergyWh = \MeasuredFrameEnergyJoules$).
