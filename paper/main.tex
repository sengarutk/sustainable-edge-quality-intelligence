...
\caption{Net carbon waterfall decomposition ($\Delta C = \Delta C_{\text{mat}} + \Delta C_{\text{rw}} + \Delta C_{\text{rev}} - C_{\text{edge}} + \Delta C_{\text{esc}}$) across Scenarios A, B, and C under Policy $B4$. Defect escape avoidance ($\Delta C_{\text{esc}}$) constitutes the primary positive contributor ($+48.0$\,kgCO$_2$e in A, $+208.0$\,kgCO$_2$e in B, $+1800.0$\,kgCO$_2$e in C), while continuous edge compute electricity ($-C_{\text{edge}} = -0.071$\,kgCO$_2$e) forms a persistent negative offset.}
